...
Betweeen all this, I burned many cycles (like 30 minutes) trying to further "optimize" the code to use discord search api via prompt rather than just scraping everything, since that seemed like an obvious answer to reduce api calls to discord for this this specific problem. However, based on my bot prompt interactions, it seems like DISCORD BOTS themselves, even if IDENTITY VALIDATED BY STRIPE ADMINS or whatever the fuck shit I had to go through to even get the goddamned discord bot token in the first place, are fucking BANNED from using the search api.
